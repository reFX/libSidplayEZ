//
// This file contains chip-profiles which allow us to adjust
// certain settings that varied wildly between 6581 chips, even
// made in the same factory on the same day.
//
// This works under the assumption that the authors used the
// same SID chip their entire career.
//
{ "20th Century Composers",				{	.folder = "/MUSICIANS/0-9/20CC/",					.filter = 0.4	} },
{ "Anthony Lees",						{	.folder = "/MUSICIANS/L/Lees_Anthony/",				.filter = 1.3	} },
{ "Antony Crowther (Ratt)",				{	.folder = "/MUSICIANS/C/Crowther_Antony/",			.filter = 1.1,	} },
{ "Barry Leitch (The Jackal)",			{	.folder = "/MUSICIANS/L/Leitch_Barry/",				.filter = 0.3,	.digi = 1.5	} },
{ "Ben Daglish",						{	.folder = "/MUSICIANS/D/Daglish_Ben/",				.filter = 0.6,	.exceptions = { { "Last_Ninja", "Anthony Lees" } } } },	// Only Anthony Lees used the filter
{ "Carsten Berggreen (Scarzix)",		{	.folder = "/MUSICIANS/S/Scarzix/",					.filter = 0.7	} },
{ "Charles Deenen",						{	.folder = "/MUSICIANS/D/Deenen_Charles/",			.filter = 0.2	} },
{ "Chris Hülsbeck",						{	.folder = "/MUSICIANS/H/Huelsbeck_Chris/",			.filter = 0.9,	.digi = 1.1,	.zeroDac = 0.2	} },
{ "Clever Music",						{	.folder = "/MUSICIANS/C/Clever_Music/",				.filter = 0.25	} },
{ "David Dunn",							{	.folder = "/MUSICIANS/D/Dunn_David/",				.filter = 0.1,	.zeroDac = 0.9 } },
{ "David Whittaker",					{	.folder = "/MUSICIANS/W/Whittaker_David/",			.filter = 0.25	} },
{ "Edwin van Santen",					{	.folder = "/MUSICIANS/0-9/20CC/van_Santen_Edwin/",	.filter = 0.5	} },
{ "Falco Paul",							{	.folder = "/MUSICIANS/0-9/20CC/Paul_Falco/",		.filter = 0.15	} },
{ "Figge Wasberger (Fegolhuzz)",		{	.folder = "/MUSICIANS/F/Fegolhuzz/",				.filter = 0.25	} },
{ "Fred Gray",							{	.folder = "/MUSICIANS/G/Gray_Fred/",				.filter = 0.4,	.zeroDac = 0.1	} },
{ "Geir Tjelta",						{	.folder = "/MUSICIANS/T/Tjelta_Geir/",				.filter = 0.5,	.digi = 1.3		} },
{ "Georg Feil",							{	.folder = "/MUSICIANS/F/Feil_Georg/",				.filter = 0.2	} },
{ "Glenn Gallefoos",					{	.folder = "/MUSICIANS/B/Blues_Muz/Gallefoss_Glenn/",.filter = 1.3,	.zeroDac = 0.5	} },
{ "Jason Page",							{	.folder = "/MUSICIANS/P/Page_Jason/",				.filter = 0.35	} },
{ "Jeroen Tel",							{	.folder = "/MUSICIANS/T/Tel_Jeroen/",				.filter = 0.35,	.digi = 1.0,	.zeroDac = 0.6	} },
{ "Johannes Bjerregaard",				{	.folder = "/MUSICIANS/B/Bjerregaard_Johannes/",		.filter = 0.35, .zeroDac = 0.6,	.exceptions = { { "Stormlord", "Jeroen Tel"	} } } },
{ "Jonathan Dunn",						{	.folder = "/MUSICIANS/D/Dunn_Jonathan/",			.filter = 0.25,	.digi = 0.5	} },
{ "Jori Olkkonen (Yip)",				{	.folder = "/MUSICIANS/Y/Yip/",						.filter = 0.35,	.zeroDac = 0.6	} },
{ "Jouni Ikonen (Mixer)",				{	.folder = "/MUSICIANS/M/Mixer/",					.filter = 0.25	} },
{ "Kim Christensen (Future Freak)",		{	.folder = "/MUSICIANS/F/Future_Freak/",				.filter = 0.35	} },
{ "Laxity",								{	.folder = "/MUSICIANS/L/Laxity/",					.filter = 0.3	} },
{ "Linus Åkesson (lft)",				{	.folder = "/MUSICIANS/L/Lft/",						.filter = 0.3	} },
{ "Mark Cooksey",						{	.folder = "/MUSICIANS/C/Cooksey_Mark/",				.filter = 0.4,	.zeroDac = 0.7	} },
{ "Mark Wilson",						{	.folder = "/MUSICIANS/W/Wilson_Mark/",				.filter = 0.2	} },
{ "Markus Müller",						{	.folder = "/MUSICIANS/M/Mueller_Markus/",			.filter = 0.5,	.zeroDac = 0.3	} },
{ "Martin Galway",						{	.folder = "/MUSICIANS/G/Galway_Martin/",			.filter = 0.65,	.zeroDac = 0.6 } },
{ "Martin Walker",						{	.folder = "/MUSICIANS/W/Walker_Martin/",			.filter = 0.15	} },
{ "Matt Gray",							{	.folder = "/MUSICIANS/G/Gray_Matt/",				.filter = 0.3	} },
{ "Michael Hendriks",					{	.folder = "/MUSICIANS/F/FAME/Hendriks_Michael/",	.filter = 0.35,	.digi = 2.0,	.zeroDac = 0.7	} },
{ "Michael Nilsson-Vonderburgh (Mitch)",{	.folder = "/MUSICIANS/M/Mitch_and_Dane/Mitch/",		.filter = 0.3,	.zeroDac = 0.2	} },
{ "Mitch & Dane",						{	.folder = "/MUSICIANS/M/Mitch_and_Dane/",			.filter = 0.85,	.zeroDac = 0.3,	.cwsThreshold = 0.5	} },
{ "Neil Brennan",						{	.folder = "/MUSICIANS/B/Brennan_Neil/",				.filter = 0.25	} },
{ "NM156",								{	.folder = "/MUSICIANS/N/NM156/",					.filter = 0.7,	.zeroDac = 0.8	} },
{ "Paul Hannay (Feekzoid)",				{	.folder = "/MUSICIANS/F/Feekzoid/",					.digi = 0.8 } },
{ "Peter Clarke",						{	.folder = "/MUSICIANS/C/Clarke_Peter/",				.filter = 0.2	} },
{ "Pex Tufvesson",						{	.folder = "/MUSICIANS/M/Mahoney/",					.filter = 0.35	} },
{ "Ramiro Vaca",						{	.folder = "/MUSICIANS/V/Vaca_Ramiro/",				.filter = 0.7,	} },
{ "Reyn Ouwehand",						{	.folder = "/MUSICIANS/O/Ouwehand_Reyn/",			.filter = 0.8,	.digi = 0.9,	.zeroDac = 0.2	} },
{ "Richard Joseph",						{	.folder = "/MUSICIANS/J/Joseph_Richard/",			.filter = 0.3	} },
{ "Rob Hubbard",						{	.folder = "/MUSICIANS/H/Hubbard_Rob/",				.filter = 0.35,	.digi = 0.7,	.zeroDac = 0.6	} },
{ "Russell Lieblich",					{	.folder = "/MUSICIANS/L/Lieblich_Russell/",			.filter = 0.25,	.zeroDac = 0.7	} },
{ "Stellan Andersson (Dane)",			{	.folder = "/MUSICIANS/M/Mitch_and_Dane/Dane/",		.filter = 0.85,	.zeroDac = 0.3	} },
{ "Steve Turner",						{	.folder = "/MUSICIANS/T/Turner_Steve/",				.filter = 0.6,	.exceptions = { { "Bushido", "Jason Page"	} } } },	// Only Jason Page used the filter for subtune 2
{ "Thomas Mogensen (DRAX)",				{	.folder = "/MUSICIANS/D/DRAX/",						.filter = 0.3	} },
{ "Tim Follin",							{	.folder = "/MUSICIANS/F/Follin_Tim/",				.filter = 0.5	} },
{ "Zoci-Joe",							{	.folder = "/MUSICIANS/Z/Zoci-Joe/",					.filter = 0.3	} },
